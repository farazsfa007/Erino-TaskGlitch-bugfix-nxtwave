...
function AppContent() {
  // [BUG 2 FIX] Destructure 'clearLastDeleted' from context
  const { 
    loading, 
    error,
    derivedSorted, 
  ...
  // [BUG 2 FIX] Actually clear the state when snackbar closes
  const handleCloseUndo = (event?: React.SyntheticEvent | Event, reason?: string) => {
    if (reason === 'clickaway') {
      return;
    }
    clearLastDeleted();
